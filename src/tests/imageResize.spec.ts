...
describe("Image Resize Process", () => {
    ...
    const fileOptions: FileOptions = {
        filename,
        thumbName: `${resizeOptions.width}_${resizeOptions.height}_${resizeOptions.fit || "cover"}_${resizeOptions.position || "centre"
            }_${filename}`,
        thumbsFolder: path.resolve("assets", "thumb"),
    }

    const fullPath: string = path.resolve(fileOptions.thumbsFolder, fileOptions.thumbName)
    ...
    it("Should return image with the specified dimensions", async () => {
        ...
        const outputInfo: OutputInfo = await imageResize(resizeOptions, fileOptions)
